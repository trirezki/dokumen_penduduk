<template>
  <div v-if="!loading">
    <h4 class="fw-bold py-3 mb-4" style="text-transform: capitalize">
      {{ is_form_update ? "Update" : "Tambah" }} Akun Kecamatan
    </h4>
    <div class="row">
      <div class="col-md-12">
        <div class="card mb-4">
          ...
          <form id="formAccountSettings" method="POST" @submit.prevent="save">
            <div class="card-body">
              ...
              <div class="mt-2">
                ...
                <button type="button" @click="$router.push({ name: 'AccountKecamatan' })"
...
export default {
  ...
  mounted() {
    if (this.is_form_update) {
      this.loading = true;
      axios.get(`${this.url}/backend/account/sub_district/${this.$route.params.id}`).then((response) => {
        ...
        this.$router.push({ name: 'AccountKecamatan' });
      })
    }
  },
  methods: {
    ...
    save() {
      ...
      if (!this.is_form_update) {
        axios
          .post(`${this.url}/backend/account/sub_district`, formData)
          .then((response) => {
            Swal.fire({
              icon: "success",
              title: "Berhasil",
        ...
        axios
          .post(`${this.url}/backend/account/sub_district/${this.institution.id}`, formData)
          .then((response) => {
            Swal.fire({
              icon: "success",
              title: "Berhasil",
